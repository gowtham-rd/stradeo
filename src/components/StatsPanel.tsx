...
export default function StatsPanel() {
  ...
  return (
    <div className="bg-stradeo-bg2 border border-stradeo-line rounded-[18px] p-5 mb-5 animate-fade-in">
      <div className="flex justify-around mb-5">
        <div className="text-center"><div className="text-[22px] font-bold text-stradeo-accent">{weekTotal}</div><div className="text-[11px] text-stradeo-inkdim">This week</div></div>
        <div className="text-center"><div className={`text-[22px] font-bold ${weekAcc >= 80 ? 'text-stradeo-green' : weekAcc >= 50 ? 'text-stradeo-accent' : 'text-stradeo-accent2'}`}>{weekAcc}%</div><div className="text-[11px] text-stradeo-inkdim">Accuracy</div></div>
        <div className="text-center"><div className="text-[22px] font-bold text-stradeo-accent">🔥 {progress.streak}</div><div className="text-[11px] text-stradeo-inkdim">Streak</div></div>
      </div>

      <div className="text-xs font-bold text-stradeo-inkdim uppercase tracking-wider mb-2.5">Last 7 days</div>
      ...
      <div className="flex gap-1.5">
        {dayLabels.map((l, i) => (
          <div key={i} className={`flex-1 text-center text-[10px] font-semibold ${days[i] === todayStr ? 'text-stradeo-accent' : 'text-stradeo-inkdim'}`}>{l}</div>
        ))}
      </div>

      {weakestTopics.length > 0 && (
        <>
          <div className="text-xs font-bold text-stradeo-inkdim uppercase tracking-wider mt-5 mb-2.5">Weakest topics</div>
          {weakestTopics.map(wt => (
            <div key={wt.id} className="flex items-center gap-2.5 mb-1.5">
              ...
              <Link href={`/topic?id=${wt.id}`} className="px-2.5 py-1 rounded-md border border-stradeo-line bg-stradeo-surface2 text-stradeo-accent text-[11px] font-semibold">Practice</Link>
            </div>
          ))}
        </>
      )}
